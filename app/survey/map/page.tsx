...
export default async function SurveyMapPage({ searchParams }: SurveyMapPageProps) {
  ...
  const { data, error } = await supabase
    ...
    .select(
      'id,request_no,customer_name,phone,request_type,request_intent,meter_size,phase,area_name,assignee_name,assigned_surveyor_id,assigned_surveyor,scheduled_survey_date,survey_date_initial,survey_date_current,status,latitude,longitude,created_at'
    )
    .in('status', statuses)
