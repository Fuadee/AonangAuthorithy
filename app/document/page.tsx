...
export default async function DocumentQueuePage({ searchParams }: DocumentQueuePageProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold">คิวเอกสาร</h2>
        <p className="mt-1 text-sm text-slate-500">แสดงงานย่อยในคิวเอกสาร พร้อมกรองตามสถานะย่อยได้</p>
      </div>

      <section className="card p-4">
        <div className="flex flex-wrap gap-2">
          <Link className={`rounded-full border px-3 py-1.5 text-sm ${selectedStatus === 'ALL' ? 'border-brand-600 bg-brand-50 text-brand-700' : 'border-slate-300 bg-white text-slate-600'}`} href="/document">
